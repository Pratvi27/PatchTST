#!/usr/bin/env bash
# Run iTransformer on Weather across 4 horizons {96, 192, 336, 720}, with the
# settings aligned to our PatchTST reproduction:
#   - train_epochs = 20            (official default is 10)
#   - same csv as PatchTST         (symlinked into iTransformer/dataset/weather/)
#   - itr = 1                      (single run; no triple-experiment averaging)
#
# All other hyperparameters follow scripts/multivariate_forecasting/Weather/iTransformer.sh
# (d_model=512, d_ff=512, e_layers=3, batch_size=default, lr=default).
#
# This script does NOT modify any official file. Drop-in addition only.
#
# Usage:
#   bash run_weather_compare.sh                      # all 4 horizons
#   bash run_weather_compare.sh "96 192"             # subset of horizons
#
# Env overrides:
...
# ─── Config ────────────────────────────────────────────────────────────
CSV_PATH="${CSV_PATH:-/model/patchTST/all_six_datasets/weather/weather.csv}"
EPOCHS="${EPOCHS:-20}"
SEQ_LEN="${SEQ_LEN:-96}"
...
# Weather has 21 variables.
VARS=21

export CUDA_VISIBLE_DEVICES="$GPU"

# ─── Symlink csv into the location iTransformer expects ────────────────
mkdir -p "${SCRIPT_DIR}/dataset/weather"
if [[ ! -f "$CSV_PATH" ]]; then
    echo "ERROR: csv not found at $CSV_PATH" >&2
    echo "       Override with: CSV_PATH=/your/path bash $0" >&2
    exit 1
fi
ln -sfn "$CSV_PATH" "${SCRIPT_DIR}/dataset/weather/weather.csv"
echo "Symlinked: ${SCRIPT_DIR}/dataset/weather/weather.csv -> $CSV_PATH"

# ─── Output dir (lives inside iTransformer/, alongside checkpoints/) ──
TS=$(date +%Y%m%d-%H%M%S)
SUMMARY_DIR="${SCRIPT_DIR}/result/weather_e${EPOCHS}_${TS}"
mkdir -p "$SUMMARY_DIR"

echo "============================================================"
echo " iTransformer Weather sweep (aligned with PatchTST settings)"
echo "------------------------------------------------------------"
echo "  csv      : $CSV_PATH"
echo "  epochs   : $EPOCHS"
